...
class NotificationListener : NotificationListenerService() {
    ...
    fun reply(msgID: String?, message: String, exception: Exception.() -> Unit = {}) {
        try {
            val action = actions.getOrDefault(msgID, null)
            if (action != null) {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        // Process the incoming notification here
            try {
                if (sbn != null) {
                    actions[sbn.notification.shortcutId] = sbn.notification.actions.find { it.title == "Reply"  || it.title.toString().equals("quick reply", true)}
    ...
    private fun getNotiData(sbn: StatusBarNotification): NotificationsModel {
        ...
        intent.putExtra(MSG_ID, sbn.notification.shortcutId)
    ...
    private fun isDeviceLocked(context: Context): Boolean {
        ...
        val isLocked = if (inKeyguardRestrictedInputMode) {
            ...
            !(context.getSystemService(POWER_SERVICE) as PowerManager).isInteractive
        }
        return isLocked
    }
