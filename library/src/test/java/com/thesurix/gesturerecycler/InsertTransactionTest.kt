...
class InsertTransactionTest : BaseTransactionTest() {
    ...
    fun insertItemWithoutHeader() {
        ...
        val isSuccessful = transaction.perform(transactional)
    ...
    @Test
    fun revertInsertItemWithoutHeader() {
        val itemToInsert = "X" // Define the item to be inserted
        ...
        val isReverted = transaction.revert(transactional)
    ...
    fun insertItemWithHeader() {
        ...
        val isSuccessful = transaction.perform(transactional)
        assertTrue(isSuccessful)
        assertEquals(transactional.data[5], itemToInsert) // Verify that the inserted item is at the correct position (5, considering the header)
    ...
    @Test
    fun revertInsertItemWithHeader() {
        val itemToInsert = "X" // Define the item to be inserted
        val itemBeforeInsert = transactional.data[4] // Save the item at the position before insertion
        val transaction = InsertTransaction(itemToInsert, 4, true) // Create a new insert transaction with the item, position, and header
        transaction.perform(transactional) // Perform the transaction
        ...
        assertTrue(isReverted)
        assertEquals(transactional.data[4], itemBeforeInsert) // Verify that the original item is restored
        Mockito.verify(transactional).notifyRemoved(5) // Check if the notifyRemoved method is called with the correct position (5, considering the header)
    }
}
